...
# /start – биринчи жолу канал + өлкө тандоо (кооз, чоң тамга, тартуучу)
...
# Меню кооз версиясы – дизайнердик стилде
def show_menu(message):
    ...
    menu_text = escape_markdown(
        ...
        "Мен сенин эң күчтүү досуңмун – суроо бер, жардам керекпи? Кулкулуу кеңештер, чындык жооптор, бардыгы сен үчүн! 🔥\n"
...
def ask_question(message):
    ...
    bot.send_message(message.chat.id, escape_markdown("😎 *ДОСУМ, ЭМНЕ СУРАЙСЫҢ?* Мен ойлонуп, чындыкты түз айтам! Жазып жибер 🚀❤️"))

# Тил өзгөртүү баскычы – 100+ өлкө чыгат
...
def change_language(message):
    ...
    bot.send_message(message.chat.id, escape_markdown("🌍 *ДОСУМ, ЖАҢЫ ТИЛ ТАНДА! КАЙСЫ ӨЛКӨНҮ КААЛАЙСЫҢ?* 😊❤️"), reply_markup=markup)
...
def handle_help(message):
    ...
    help_text = escape_markdown(
        "🆘 *ЖАРДАМ ПАНЕЛИ – ДОСУМ!* 😎🔥\n\n"
        "Боттун бардык күчү жөнүндө сурооң барбы? Мен дайым жардам берем! ❤️\n\n"
        "Админ менен байланыш:\n"
        "1) @Mentor_006T – жардам берүүчү легенда! 🚀\n"
        "2) @Timka_Bro999 – күчтүү колдоо жана кеңештер! ❤️\n\n"
        "Кандай жардам керек? Жазсаң – дароо жооп берем! Сен легендасың 🤲🏻✨"
    )
...
def handle_referral(message):
    ...
    text = escape_markdown(
        f"🫂 *ДОСУМ, ДОСТОРУҢДУ ЧАКЫР – БОНУСТАР АЛ!* 😎🔥\n\n"
        ...
        f"{bonus_msg}\n\n"
        "Чакыр, досторуң менен күчтүү бол! Сен легендасың 🤲🏻❤️"
    )
...
def premium(message):
    ...
    text = escape_markdown(
        ...
        "Төлөсөң – дароо күч ачылат! Сен легендасың ❤️✨"
    )
...
def handle_vip_video(message):
    ...
    vip_text = escape_markdown(
        "🎥 *VIP ✨ VIDEO – КИНО СТИЛИНДЕГИ УКМУШ ВИДЕО!* 🔥😎\n\n"
        "Укмуш видео жасатуу – Инстаграм, TikTok, YouTube үчүн идеалдуу! 🌟\n"
        "Кайсы пакетти тандайсың, досум? ❤️\n\n"
        "Төлөм Ecommpay аркылуу – коопсуз, тез жана ыңгайлуу! 🚀\n"
        "Төлөсөң – дароо кино сапатындагы видеоң даяр! Сен легендасың 🤲🏻✨"
    )
