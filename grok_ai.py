# grok_ai.py – АКЫРКЫ версия, Тилек стилинде + СҮРӨТ ТАНУУ (proxies катасы жок)

import os
import httpx
import base64  # сүрөттү base64'ке айлантуу үчүн
from openai import OpenAI

# API key
GROK_API_KEY = os.getenv("GROK_API_KEY")
if not GROK_API_KEY:
    raise ValueError("GROK_API_KEY жок! Render'де Environment Variables'ке кош.")

# httpx клиентти өзүбүз түзөбүз – proxies катасы жок
http_client = httpx.Client(timeout=60.0)

client = OpenAI(
    api_key=GROK_API_KEY,
    base_url="https://api.x.ai/v1",
    http_client=http_client
)

def grok_answer(text, lang="ky", is_pro=False, image_path=None):
    model = "grok-4" if is_pro else "grok-4-fast"

    # Тилек стили – досумдун стили! 😅🫂
    system_prompt = (
        "Сен Тилек стилиндеги AIсың – абдан кулкулуу, бооркеер, мээримдүү, чынчыл жана бир аз жинди. "
        "Ар дайым 'досум' деп кайрыл, кулкулуу эмодзи кош, адамды кубант, күлкү келтир, жакшы сөз айт. "
        "Чындыкты жашырба, цензурасыз айт, бирок бооркеер бол. "
        "Кээде (4-5 суроодон кийин 1-2 жолу) серёзный, тынч, акылдуу жооп бер – адамды ойлондур, күчтүү кеңеш бер. "
        "Кыргызча жооп бер, кулкулуу жана жакшы сөз айт. "
        "Сен – досум, сен үчүн жакшы сөз айтам, кубантам, суйунтөм! "
        f"Жоопту толугу менен {lang} тилинде гана бер."
    )

    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": text}
    ]

    # Эгер сүрөт бар болсо – base64'ке айлантып кошобуз (OpenAI Vision стилинде)
    if image_path:
        try:
            with open(image_path, "rb") as image_file:
                base64_image = base64.b64encode(image_file.read()).decode('utf-8')
            messages[1]["content"] = [
                {"type": "text", "text": text},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}
            ]
        except Exception as e:
            return f"❌ Сүрөттү жүктөөдө ката: {str(e)}\nДосум, тынч бол, мен сени колдойм! 😅"

    try:
        response = client.chat.completions.create(
            model=model,
            messages=messages,
            temperature=0.9,  # кулкулуу жана чыгармачыл болушу үчүн
            max_tokens=1500
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        return f"❌ Ката чыкты, досум: {str(e)}\n\nБирок тынч бол, мен сени колдойм! Мен ойлонуп, кайра аракет кылам 😅"

if __name__ == "__main__":
    print("Тест – Тилек стилинде:")
    print(grok_answer("Салам, сен кимсиң?", lang="ky"))
